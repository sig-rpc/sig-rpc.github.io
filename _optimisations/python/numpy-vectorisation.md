...
The manner by which NumPy stores data in arrays enables it's functions to utilise vectorisation, whereby the processor executes one instruction across multiple variables simultaneously, for every mathematical operation between arrays. Vectorised maths can be many times faster, however it requires using supported functions.
...
Additionally, NumPy's vectorised functions can sometimes take advantage of auto parallelisation, particularly on HPC systems.
