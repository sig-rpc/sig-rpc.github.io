...
Being a larger problem I opted to profile parallel code, as such I used an [Intel VTune](/profiler/cpp/intel-vtune/) hotspots profile/
...
The above snippet from the summary report from command-line Intel VTune clearly identified `LB::smoothenInterface` as the most expensive function. When located, the function contained a comment showing the author had previously identified this as harmful to the performance.

I've briefly summarised the algorithm, which removes neighbours of `emptiedNodes` from `fluidNodes` (as they will become `interfaceNodes`) in pseudo-python below:
...
To optimise this, we can replace that inner loop, instead building a set of `fluidNode` to be deleted. After the main outer loop completes, we have a set of unique nodes to be deleted, so we can iterate `fluidNodes` once, deleting any nodes that appear in the set. Checking whether a node exists within our set of nodes to be deleted, should almost always be faster than iterating that same data in a list or array ([changes shown here](https://github.com/gnomeCreative/HYBIRD/commit/f86ab23640b59c9befe5e29290359d4e40789245)).

**This optimisation with a set data-structure, reduced the runtime of the profiled example from 6h51m to 2h23m. A 2.87x speedup to the parallel code!**

## Small IO Operations

It was then suggested that I profile the Tsunami Wave tutorial, this was the first example I profiled that combined both DEM and LBM.

To profile this, again I used an [Intel VTune](/profiler/cpp/intel-vtune/) hotspots profile on the multi-threaded code.



The above snippet from the summary report from command-line Intel VTune above show `LB::smoothenInterface`

It's worth noting that there are different approaches for formatting strings, with difference performance consequences. [fmt](https://fmt.dev/) has [benchmarked many approaches for double-to-string conversion](https://github.com/fmtlib/dtoa-benchmark), and found `std::ostream` as used here to be the slowest! With many other libraries being 40x faster. Regardless, writing binary data to disk free conversion will always be faster.

[Documentation for the VTK file format](https://docs.vtk.org/en/latest/design_documents/VTKFileFormats.html) is somewhat lacking, presumably as they intend for you to use the IO API built into VTK. The format of the files is essentially XML, with the binary variant having an XML header and binary blob footer. It took a bit of trial and error to reverse-engineer the correct memory layout of the binary VTK files. Then over 3 iterations I optimised a new binary VTK export function, this one minimised the write calls by building each buffer in memory before writing it to file. When benchmarking the Tsunami Wave tutorial, **this optimisation reduced the runtime from 4h09m to 3h31m. A 1.30x speedup to the parallel code!**

Furthermore, with binary files both being more efficient to read and write, it reduced the time to load large frames into Paraview. **Roughly timed under Paraview 6.0, originally an ASCII frame from the tsunami wave tutorial, would take around 20 seconds to load, in comparison binary frames now load in closer to half a second.** This also impacts researcher productivity when processing results. The video below shows Paraview loading the frames as fast as possible, imagine waiting 20 seconds per frame!

<iframe width="560" height="315" src="https://www.youtube.com/embed/BfiTDHJrxlE?si=-40rIyPuF5qXQ4pR" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

## OpenMP

Alongside this profiling and optimisation work, there were several scattered improvements to the OpenMP within the project. These took the form of typical bad habits; overuse of expensive critical sections, and creating many independent parallel sections rather than creating less and sharing them to reduce the creation overhead. **Overall the impact of these changes was quite minor at scale, relative to other optimisations.**
...
To provide a summary comparison I benchmarked the [tsunami wave tutorial](https://github.com/gnomeCreative/HYBIRD/wiki/Tutorial-5), which is the largest of the provided examples and combines both DEM and LBM, with both the pre- and post-optimisation code. Before optimisation it took 26h14m to execute to completion, and afterwards it took 4h15m. This equates to a combined 6.17x speed-up to that particular example. As previously discussed, these speed ups are highly likely to vary across problems due to the nature of the optimisations. Additionally, any combined DEM and LBM problem, such as tsunami wave, will have orders of magnitude less DEM particles than LBM nodes thus reducing the relevance of the DEM specific optimisations.

Further to this speed-up, the replacement of ASCII with binary output files would see the previously mentioned approximately 40x improvement to Paraview's loading speed. Additionally improving research productivity when processing the results.
